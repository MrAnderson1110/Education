...
public:
    ...
    GLMatrix(const GLMatrix<size> &rightMat) { memcpy(m_data, rightMat.m_data, sizeof(GLfloat) * size * size); }
    GLMatrix(const GLfloat *arr) { memcpy(m_data, arr, sizeof(GLfloat) * size * size); }
    ...
    GLMatrix(const std::initializer_list<GLfloat> &list)
    {
        if(list.size() > size * size)
            qFatal("initializer list size not equal defined matrix size");
        memcpy(m_data, list.begin(), sizeof(GLfloat) * size * size);
    }

//!TODO реализовать эти методы
//    static GLMatrix<size> createRotMatrix(const GLVector<size> &vec, const GLfloat &angle) {}
...
//    static GLMatrix<size> createTransMatrix(const GLfloat &x, const GLfloat &y, const GLfloat &z) {}
//    static GLMatrix<size> createScaleMatrix(const GLfloat &scaleFactor) {}
    ...
    GLMatrix<size> operator*(const GLfloat &rightVal)
    {
        GLMatrix<size> result;
        for(int i = 0; i < size * size; ++i)
            result[i] *= rightVal;
    ...
    const GLfloat &operator[](size_type index) const
    {
        return m_data[index];
    }
    GLfloat &operator[](size_type index)
    {
        return m_data[index];
    }
    ...
    const GLfloat *toData() const { return m_data; }
